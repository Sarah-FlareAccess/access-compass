...
const featureInfoContent: Record<string, { title: string; description: string; examples?: string[] }> = {
  ...
  superuserTraining: {
    title: 'Access Compass Superuser Program',
    description: 'Your own people, trained to run Access Compass properly, so the capability sits in your organisation rather than in one person\'s head. When someone moves on, the next one is trained and the plan keeps running. That is the difference between buying software and building capability, and it is the answer to the problem this whole platform exists for. Larger editions add live workshops and ongoing training as your team changes.',
    examples: [
      ...
      'Separate to advisory — this is how to run the platform, not what to do about accessibility',
    ],
  },
  integrations: {
    title: 'Works With Your Project Tools',
    description: 'Access Compass is a complete accessibility governance platform. It includes its own action management workspace, so most teams need nothing else. If your organisation already has established project management systems, Access Compass can also work alongside them: send each team a delivery package containing their actions, owners, due dates, success indicators, evidence requirements and framework references, to load into whatever they already use. The evidence comes back here, because that is the part a task tool cannot hold and your board report cannot do without. Your choice, team by team. Nothing locked in either way.',
    examples: [
      'Run the whole plan in Access Compass, with no other tool involved',
      'Send Parks their 17 actions as one file, with owners, due dates and the evidence required',
      'Facilities work the job in Asana; the proof it happened lands back in Access Compass',
  ...
  teamAllocation: {
    ...
    description: 'Assign modules and action plan items to the team members responsible for them.',
  },
  ...
  multiDiap: {
    ...
    description: 'Your action plan runs in cycles, and when a new one starts the old one does not stop mattering. Keep each completed plan and compare against it, so when someone asks what happened to the last plan you can answer with the record rather than from memory. This is the difference between tracking work and holding a governance history across a statutory cycle.',
    examples: [
      'DIAP 2021-2025 against DIAP 2025-2029: what we committed to, what we delivered',
      'Evidence a full cycle of progress to elected members or your community',
      'Hand your successor the history, not just the current list',
    ],
  },
  dataResidency: {
...
const featureLabelsMultiSite: { key: keyof TierFeatures; label: string; infoKey?: string }[] = [
  ...
  { key: 'report', label: 'Accessibility Report & Recommendations' },
  { key: 'diap', label: 'Disability Inclusion Action Plan (DIAP)', infoKey: 'diap' },
  { key: 'teamAllocation', label: 'Team Allocation', infoKey: 'teamAllocation' },
  { key: 'evidenceLibrary', label: 'Evidence Library' },
...
const featureLabelsOrgAccessibility: { key: keyof TierFeatures; label: string; infoKey?: string }[] = [
  ...
  { key: 'multiDiap', label: 'Program History & Cycle Comparison', infoKey: 'multiDiap' },
  { key: 'report', label: 'Executive & Board Reporting', infoKey: 'stakeholderReporting' },
  { key: 'sso', label: 'Single Sign-On (SSO)', infoKey: 'sso' },
  { key: 'evidenceLibrary', label: 'Evidence Library' },
  { key: 'teamAllocation', label: 'Team Allocation', infoKey: 'teamAllocation' },
  { key: 'superuserTraining', label: 'Superuser Program', infoKey: 'superuserTraining' },
  { key: 'integrations', label: 'Works With Your Project Tools', infoKey: 'integrations' },
