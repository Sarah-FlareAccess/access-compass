...
export default function DIAPWorkspace() {
  ...
  const filtersApplied = useMemo(() => {
    const nonSite = filterStatuses.size > 0 || filterCategories.size > 0
      || filterPriorities.size > 0 || filterDueDate.size > 0 || filterResponsible !== 'all';
  ...
  // Auto-generate a venue's action plan the first time it's opened with
  // completed modules but no actions yet. Debounced so it waits for the site's
  // module progress to settle after a venue switch (avoids tagging one venue's
  // findings to another). Tracked per venue so deleting a plan doesn't trigger
  // an endless regenerate; the manual banner remains as a fallback.
  const autoGeneratedSites = useRef<Set<string>>(new Set());
  useEffect(() => {
    const key = activeSiteId ?? '__org__';
    if (isLoading || isGenerating) return;
    if (autoGeneratedSites.current.has(key)) return;
    if (completedModulesEvidence.length === 0 || siteScopedItems.length > 0) return;
    const timer = setTimeout(() => {
      autoGeneratedSites.current.add(key);
          ...
          <div className="domain-tag-field">
            ...
            <details className="domain-tag-dropdown" ref={domainDropdownRef}>
              <summary aria-label="Select statutory outcomes">
                {domainTags.length ? (
                  <span className="domain-tag-value">{domainTags.length} selected</span>
                ) : (
