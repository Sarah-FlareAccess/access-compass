...
export default function ReportPage() {
  ...
  const coveredSiteNames = useMemo(
    () => (isOrgWideMultiSite ? sites.map(s => s.name).filter(Boolean) : undefined),
    [isOrgWideMultiSite, sites]
  );
  ...
  const handleGenerateReport = useCallback(() => {
    if (!isReady) return;
    const r = generateReport(reviewMode, organisationName, reportConfig, activeSiteName, jurisdiction, coveredSiteNames);
    setReport(r);
  }, [isReady, generateReport, reviewMode, organisationName, reportConfig, activeSiteName, jurisdiction, coveredSiteNames]);
  ...
  }, [isReady, hasCompletedModules, hasAutoGenerated, generateReport, reviewMode, organisationName, reportConfig, activeSiteName, jurisdiction, coveredSiteNames]);
  ...
  useEffect(() => {
    ...
    setReport(generateReport(reviewMode, organisationName, reportConfig, activeSiteName, jurisdiction, coveredSiteNames));
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [activeSiteName, generateReport, jurisdiction, coveredSiteNames]);
      ...
      <div className="rp-container">
        ...
        {report.coveredSites && report.coveredSites.length > 0 && (
          <div className="rp-scope">
            <span className="rp-scope-label">This organisation-wide report aggregates self-review assessments across {report.coveredSites.length} {report.coveredSites.length === 1 ? 'venue' : 'venues'}:</span>
            <span className="rp-scope-list">{report.coveredSites.join(', ')}</span>
          </div>
        )}
        ...
        {/* Flare Access consulting note - closing, kept out of the report body */}
        <div className="rp-flare-note">
          <span className="rp-flare-note-label">{FLARE_CONTACT.label}</span>
          <div className="rp-flare-note-links">
            <a href={`mailto:${FLARE_CONTACT.email}`}>{FLARE_CONTACT.email}</a>
            <span className="rp-flare-note-sep" aria-hidden="true">|</span>
