...
declare
  ...
  v_rec      record;
begin
  -- One scored row per (module_progress row), counting only answered,
  -- applicable responses. Legacy aliases are folded in: 'na' predates
  -- 'not-applicable', 'not-sure' predates 'unable-to-check'.
  create temp table _cs_calc on commit drop as
  select
    mp.id,
    mp.confidence_snapshot as old_band,
    count(*) filter (
      where mr.answer is not null and mr.answer not in ('not-applicable', 'na')
    ) as total,
    count(*) filter (where mr.answer = 'yes') as n_yes,
    count(*) filter (where mr.answer = 'partially') as n_partial,
    count(*) filter (where mr.answer = 'no') as n_no,
    count(*) filter (where mr.answer in ('unable-to-check', 'not-sure')) as n_unsure
  from module_progress mp
  join module_responses mr
    on mr.session_id = mp.session_id
